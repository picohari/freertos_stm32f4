...
//
// \return Returns \e 1 if the passed character is a valid hex digit or \e
// 0 otherwise
//
//*****************************************************************************
uint8_t
...
//
// \return Returns \b 1 on success or \b 0 if pcEncoded does not point
...
//*****************************************************************************
unsigned long
EncodeFormString(const char *pcDecoded, char *pcEncoded, unsigned long ulLen)
{
        ...
        switch(pcEncoded[ulLoop])
        {
            ...
            case '%':
            {
                if(pcEncoded[ulLoop + 1] && pcEncoded[ulLoop + 2])
                {
                    ...
                    //
                    bValid = DecodeHexEscape(&pcEncoded[ulLoop], &pcDecoded[ulCount]);
...
//
// \return Returns \b 1 if the string is a valid representation of a
// decimal number or \b 0 if not.
...
// the pcParam array.
// \param pbError is a pointer that will be written to \b 1 if there is any
...
// parameter \e pbError is written to \b 1.  Note that \e pbError is NOT
...
// which case \e *pbError will be \b 1).
//
//*****************************************************************************
long
GetCGIParam(const char *pcName, char *pcParams[], char *pcValue[],
            int iNumParams, uint8_t *pbError)
{
    int iParam;
    long lValue;
    uint8_t bRetcode;
