...
//static GHandle ghCont_NetmaskHeader;
//static GHandle ghCont_NetmaskBody;
static GHandle ghCont_NetmaskFooter;
...
/* Page Prototypes */
static void SetupNetmask_onShow(void);
static void SetupNetmask_onClose(void);
static bool SetupNetmask_onEvent(MenuPageDef_t *page, GEvent *pe);
...
void create_SetupNetmask(void) {
    ...
    /* MenuNetmask container - must be present on all pages if in menu tree */
	wi.g.show = FALSE;
	wi.g.x = 0;
	wi.g.y = 0;
	wi.g.width  = 320;
	wi.g.height = 240;
	wi.g.parent = 0;
	wi.text = "Setup: Netmask";
	wi.customDraw = 0;
	wi.customParam = 0;
	wi.customStyle = 0;

	/* Menu assignments */
	menuPages[SETUP_NETMASK].title 	= wi.text;
	menuPages[SETUP_NETMASK].container = gwinContainerCreate(0, &wi, 0);
    menuPages[SETUP_NETMASK].id 		= SETUP_NETMASK;
    menuPages[SETUP_NETMASK].onShow 	= SetupNetmask_onShow;
    menuPages[SETUP_NETMASK].onClose 	= SetupNetmask_onClose;
	menuPages[SETUP_NETMASK].onEvent 	= SetupNetmask_onEvent;
	...
	wi.g.width  = 320;
	wi.g.height = 16;
	wi.g.parent = menuPages[SETUP_NETMASK].container;
	wi.text = "Edit: Netmask";
	...
	wi.customParam = 0;
	wi.customStyle = 0;
	ghCont_NetmaskFooter = gwinContainerCreate(0, &wi, 0);
	...
	wi.g.width  = 65;
	wi.g.height = 42;
	wi.g.parent = ghCont_NetmaskFooter;
...
static void SetupNetmask_onShow(void) {
    LOG_MENU("SETUP_NETMASK: onShow");
	gwinSetVisible(ghKeyboard, TRUE);

	uint32_t ipv4_address;
	axis_helper_getNetmask(&ipv4_address);
...
static void SetupNetmask_onClose(void) {
    LOG_MENU("SETUP_NETMASK: onClose");
}
    ...
    switch (pe->type) {
        ...
        case GEVENT_GWIN_BUTTON: {
            ...
            if (be->gwin == ghBtn_MenuReturn) {
            	...
            	axis_helper_setNetmask(&new_ipv4_address);

				LOG_MENU("SAVED NETMASK: 0x%08x", new_ipv4_address);
                return TRUE;
            }
