...
//static GHandle ghCont_LocalIPHeader;
//static GHandle ghCont_LocalIPBody;
static GHandle ghCont_LocalIPFooter;
...
/* Page Prototypes */
static void SetupLocalIP_onShow(void);
static void SetupLocalIP_onClose(void);
static bool SetupLocalIP_onEvent(MenuPageDef_t *page, GEvent *pe);
...
void create_SetupLocalIP(void) {
    ...
    /* MenuLocalIP container - must be present on all pages if in menu tree */
	wi.g.show = FALSE;
	wi.g.x = 0;
	wi.g.y = 0;
	wi.g.width  = 320;
	wi.g.height = 240;
	wi.g.parent = 0;
	wi.text = "Setup: LocalIP";
	wi.customDraw = 0;
	wi.customParam = 0;
	wi.customStyle = 0;

	/* Menu assignments */
	menuPages[SETUP_LOCALIP].title 	= wi.text;
	menuPages[SETUP_LOCALIP].container = gwinContainerCreate(0, &wi, 0);
    menuPages[SETUP_LOCALIP].id 		= SETUP_LOCALIP;
    menuPages[SETUP_LOCALIP].onShow 	= SetupLocalIP_onShow;
    menuPages[SETUP_LOCALIP].onClose 	= SetupLocalIP_onClose;
	menuPages[SETUP_LOCALIP].onEvent 	= SetupLocalIP_onEvent;
	...
	wi.g.width  = 320;
	wi.g.height = 16;
	wi.g.parent = menuPages[SETUP_LOCALIP].container;
	wi.text = "Edit: Local IP Address";
	...
	wi.customParam = 0;
	wi.customStyle = 0;
	ghCont_LocalIPFooter = gwinContainerCreate(0, &wi, 0);
	...
	wi.g.width  = 65;
	wi.g.height = 42;
	wi.g.parent = ghCont_LocalIPFooter;
...
static void SetupLocalIP_onShow(void) {
    LOG_MENU("SETUP_LOCALIP: onShow");
	gwinSetVisible(ghKeyboard, TRUE);

	uint32_t ipv4_address;
	//axis_helper_getLocalIP(&ipv4_address);
...
static void SetupLocalIP_onClose(void) {
    LOG_MENU("SETUP_LOCALIP: onClose");
}
    ...
    switch (pe->type) {
        ...
        case GEVENT_GWIN_BUTTON: {
            ...
            if (be->gwin == ghBtn_MenuReturn) {
            	...
            	//axis_helper_setLocalIP(&new_ipv4_address);
