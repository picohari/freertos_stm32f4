...
//static GHandle ghCont_GatewayHeader;
//static GHandle ghCont_GatewayBody;
static GHandle ghCont_GatewayFooter;
...
/* Page Prototypes */
static void SetupGateway_onShow(void);
static void SetupGateway_onClose(void);
static bool SetupGateway_onEvent(MenuPageDef_t *page, GEvent *pe);
...
void create_SetupGateway(void) {
    ...
    /* MenuGateway container - must be present on all pages if in menu tree */
	wi.g.show = FALSE;
	wi.g.x = 0;
	wi.g.y = 0;
	wi.g.width  = 320;
	wi.g.height = 240;
	wi.g.parent = 0;
	wi.text = "Setup: Gateway";
	wi.customDraw = 0;
	wi.customParam = 0;
	wi.customStyle = 0;

	/* Menu assignments */
	menuPages[SETUP_GATEWAY].title 	= wi.text;
	menuPages[SETUP_GATEWAY].container = gwinContainerCreate(0, &wi, 0);
    menuPages[SETUP_GATEWAY].id 		= SETUP_GATEWAY;
    menuPages[SETUP_GATEWAY].onShow 	= SetupGateway_onShow;
    menuPages[SETUP_GATEWAY].onClose 	= SetupGateway_onClose;
	menuPages[SETUP_GATEWAY].onEvent 	= SetupGateway_onEvent;
	...
	wi.g.width  = 320;
	wi.g.height = 16;
	wi.g.parent = menuPages[SETUP_GATEWAY].container;
	wi.text = "Edit: Gateway";
	...
	wi.customParam = 0;
	wi.customStyle = 0;
	ghCont_GatewayFooter = gwinContainerCreate(0, &wi, 0);
	...
	wi.g.width  = 65;
	wi.g.height = 42;
	wi.g.parent = ghCont_GatewayFooter;
...
static void SetupGateway_onShow(void) {
    LOG_MENU("SETUP_GATEWAY: onShow");
	gwinSetVisible(ghKeyboard, TRUE);

	uint32_t ipv4_address;
	axis_helper_getGateway(&ipv4_address);
...
static void SetupGateway_onClose(void) {
    LOG_MENU("SETUP_GATEWAY: onClose");
}
    ...
    switch (pe->type) {
        ...
        case GEVENT_GWIN_BUTTON: {
            ...
            if (be->gwin == ghBtn_MenuReturn) {
            	...
            	axis_helper_setGateway(&new_ipv4_address);

				LOG_MENU("SAVED GATEWAY: 0x%08x", new_ipv4_address);
                return TRUE;
            }
